...
{
    public class Mapper_EventManifest : Ray1TextSerializable
    {
        ...
        public override void Write(Ray1TextParser parser)
        {
            foreach (var ms in EventDefinitions.Concat<Mapper_BaseEventDefinition>(AlwaysEventDefinitions))
            {
                ...
                // Write if values
                if (ms is Mapper_AlwaysEventDefinition always && always.IfCommand?.Any() == true)
                {
                ...
                if (ms is Mapper_EventDefinition e)
