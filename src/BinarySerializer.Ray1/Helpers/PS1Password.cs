    ...
    public class PS1Password
    {
        ...
        */
        ...
        public SaveData Decode()
        {
            ...
            byte[] password = Password.ToArray();
        ...
        public class SaveData
        {
            ...
            {
                // Get data from save
                WorldMapInfo[] t_world_info = WorldInfo;
                byte finBossLevel_0 = (byte)BitHelpers.ExtractBits((ushort)FinBossLevel, 8, 0);
                byte finBossLevel_1 = (byte)BitHelpers.ExtractBits((ushort)FinBossLevel, 8, 8);
                byte lives = LivesCount;
                byte nb_continue = Continues;
                ...
                // Set data in save
                FinBossLevel = (FinBossLevel)BitHelpers.SetBits((ushort)FinBossLevel, finBossLevel_0, 8, 0);
                FinBossLevel = (FinBossLevel)BitHelpers.SetBits((ushort)FinBossLevel, finBossLevel_1, 8, 8);
                LivesCount = lives;
            ...
            public string Validate()
            {
                ...
                if (FinBossLevel.HasFlag(FinBossLevel.Moskito))
                {
                ...
                if (FinBossLevel.HasFlag(FinBossLevel.MrSax) && (!WorldInfo[7].IsUnlocked || level <= 7))
                    return $"Mr Sax has been beaten under invalid circumstances";

                if (FinBossLevel.HasFlag(FinBossLevel.HelpedMusician))
                {
                    if (level <= 9)
                ...
                if (FinBossLevel.HasFlag(FinBossLevel.MrSkops))
                {
                    if (level <= 15 || !FinBossLevel.HasFlag(FinBossLevel.HelpedMusician) || !FinBossLevel.HasFlag(FinBossLevel.Moskito))
                        return $"Mr Skops has been beaten under invalid circumstances";
                }
                else
                {
                    if (level >= 17 || FinBossLevel.HasFlag(FinBossLevel.MrDark))
                ...
                {
                    if (WorldInfo[3].HasAllCages || FinBossLevel.HasFlag(FinBossLevel.Moskito))
                ...
                {
                    if (WorldInfo[7].HasAllCages || FinBossLevel.HasFlag(FinBossLevel.MrSax))
                ...
                if (level > 8 && (!FinBossLevel.HasFlag(FinBossLevel.Moskito) || !WorldInfo[3].IsUnlocked))
                ...
                if ((level == 17) || WorldInfo[17].HasAllCages)
                {
                    ...
                    if (cages != 17 || !FinBossLevel.HasFlag(FinBossLevel.MrSkops) || !WorldInfo[3].IsUnlocked || !WorldInfo[7].IsUnlocked)
